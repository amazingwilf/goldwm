...
focus(Client *c)
{
	...
	if (c) {
		...
		if (c->scratchkey != 0 && c->isfloating)
			...
			XSetWindowBorder(dpy, c->win, scheme[SchemeTagsSel][ColBorder].pixel);
...
manage(Window w, XWindowAttributes *wa)
{
	...
	if(c->issticky)
		XSetWindowBorder(dpy, w, scheme[SchemeTagsNorm][ColBorder].pixel);
	else if (c->isfloating)
		c->bw = fborderpx;
...
unfocus(Client *c, int setfocus, Client *nextfocus)
{
	...
	if (c->scratchkey != 0 && c->isfloating)
		...
		XSetWindowBorder(dpy, c->win, scheme[SchemeTagsNorm][ColBorder].pixel);
	else if (c->isfloating)
